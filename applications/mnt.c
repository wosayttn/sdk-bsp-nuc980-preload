...
#include <sys/statfs.h>
    ...
    #define PARTITION_NAME_RTTHREAD0   "rtthread0"
    #define PARTITION_NAME_RTTHREAD1   "rtthread1"
#endif
...
int mnt_init_spiflash0(void)
{
    ...
    struct rt_device *psNorFlash = fal_blk_device_create(PARTITION_NAME_RTTHREAD0);
    if (!psNorFlash)
    {
        rt_kprintf("Failed to create block device for %s.\n", PARTITION_NAME_RTTHREAD0);
    }

    psNorFlash = fal_blk_device_create(PARTITION_NAME_RTTHREAD1);
    if (!psNorFlash)
    {
        rt_kprintf("Failed to create block device for %s.\n", PARTITION_NAME_RTTHREAD1);
    }

    psNorFlash = fal_blk_device_create(PARTITION_NAME_FILESYSTEM);
    if (!psNorFlash)
    {
